...
let numeric_error at = function
  ...
  | Eval_numeric.TypeError (i, v, t) ->
    Crash.error at
      ("type error, expected " ^ Wasm_types.string_of_value_type t ^ " as operand " ^
       string_of_int i ^ ", got " ^ Wasm_types.string_of_value_type (type_of v))
  | exn -> raise exn
...
let rec step (c : config) : config =
  ...
  let vs', es' =
    ...
    | Plain e', vs ->
      ...
      | If (ts, _es1, es2), I32 0l :: vs' ->
        vs', [Plain (Block (ts, es2)) @@ e.at]

      | If (ts, es1, _es2), I32 _i :: vs' ->
      ...
      | Select, I32 0l :: v2 :: _v1 :: vs' ->
        v2 :: vs', []

      | Select, I32 _i :: _v2 :: v1 :: vs' ->
        v1 :: vs', []
    ...
    | Label (_ts, _es0, (vs', [])), vs ->
      vs' @ vs, []

    | Label (_ts, _es0, (_vs', {it = Trapped msg; at} :: _es')), vs ->
      vs, [Trapped msg @@ at]

    | Label (ts, es0, (_vs', {it = Break (0l, vs0); at} :: _es')), vs ->
      let _ = at in
      take (List.length ts) vs0 e.at @ vs, List.map plain es0

    | Label (_ts, _es0, (_vs', {it = Break (k, vs0); at} :: _es')), vs ->
    ...
    | Frame (_frame', (_vs', {it = Trapped msg; at} :: _es')), vs ->
      vs, [Trapped msg @@ at]
