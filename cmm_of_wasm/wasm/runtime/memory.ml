...
let load_value mem a o t =
  let n = loadn mem a o (Wasm_types.size t) in
  match t with
...
let store_value mem a o v =
  let x =
    ...
    | F64 x -> F64.to_bits x
  in storen mem a o (Wasm_types.size (Values.type_of v)) x
...
let load_packed sz ext mem a o t =
  assert (mem_size sz <= Wasm_types.size t);
...
let store_packed sz mem a o v =
  assert (mem_size sz <= Wasm_types.size (Values.type_of v));
  let n = mem_size sz in
  let x =
    match v with
    | I32 x -> Int64.of_int32 x
