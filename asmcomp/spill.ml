...
let rec reload i before =
  ...
  | Ireturn | Iop(Itailcall_ind _) | Iop(Itailcall_imm _) ->
      (add_reloads (Reg.inter_set_array before i.arg) i,
       Reg.Set.empty)
  | Iop(Icall_ind _ | Icall_imm _ | Iextcall_ind { alloc = true; }
        | Iextcall_imm { alloc = true; }) ->
...
let rec spill i finally =
  ...
  | Iop _ ->
      ...
      let before =
        match i.desc with
          Iop Icall_ind _ | Iop(Icall_imm _) | Iop(Iextcall_ind _)
        | Iop(Iextcall_imm _) | Iop(Ialloc _)
